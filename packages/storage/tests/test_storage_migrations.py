...
async def _create_demo_table(conn: aiosqlite.Connection) -> None:
    await conn.execute("CREATE TABLE IF NOT EXISTS demo (id INTEGER PRIMARY KEY, name TEXT)")
...
async def test_run_pending_executes_migrations(conn: aiosqlite.Connection):
    ...
    # Verify table exists
    cursor = await conn.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='demo'")
...
async def test_migrations_table_records(conn: aiosqlite.Connection):
    ...
    cursor = await conn.execute("SELECT id, version, description FROM _migrations")
